...
public class SocialService {
	...
	// public static SocialService authenticate(Profile profile, final Class<?
	// extends Api> scribeServiceClass, String apiKey, String apiSecret, String
	// callbackURL) {
	// ServiceBuilder serviceBuilder = new
	// ServiceBuilder().provider(scribeServiceClass)
	// .apiKey(apiKey)
	// .apiSecret(apiSecret);
	//
	// if (callbackURL != null) {
	...
	//
	// Optional<ServiceCredentials> credentialOptional =
	// profile.getServices().stream().filter((ServiceCredentials credentials) ->
	// {
	...
	// Token accessToken = new Token(credential.getAccessToken(),
	...
	//
	// System.out.println("=== " + scribeServiceClass.getSimpleName() + "'s
	// OAuth Workflow ===");
	// System.out.println();
	//
	...
	// SocialStoreFactory.eINSTANCE.createServiceCredentials();
