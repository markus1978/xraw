		...
		if (userToken == null) {
			if (httpServiceConfiguration instanceof InteractiveServiceConfiguration) {
				 ...
				 * https://accounts.google.com/o/oauth2/auth?
		  		 ...
		  		 * access_type=offline
				 */
				val authorizationUrl = new StringBuilder()
					.append("https://accounts.google.com/o/oauth2/auth").append("?").append("&")		
				...
				val tokenRequest = Unirest.post("https://accounts.google.com/o/oauth2/token")
			...
			if (date > userTokenExpireTime - 10000) {
				...
				val authentication = Unirest.post("https://accounts.google.com/o/oauth2/token")
									.header("Content-Type", "application/x-www-form-urlencoded")
									.header("accept", "application/json")
									.field("client_id", apiKey)
									.field("client_secret", apiSecret)
